...
class TrackSelectionDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog { // We need to own the view to let tab layout work correctly on all API levels. We can't use
// AlertDialog because it owns the view itself, so we use AppCompatDialog instead, themed using
// the AlertDialog theme overlay with force-enabled title.
        val dialog =
            AppCompatDialog(activity, R.style.TrackSelectionDialogThemeOverlay)
    ...
    class TrackSelectionViewFragment : Fragment(),
        ...
        init { // Retain instance across activity re-creation to prevent losing access to init data.
            retainInstance = true
        }
    }
    ...
    companion object {
        ...
        fun willHaveContent(trackSelector: DefaultTrackSelector): Boolean {
            ...
            return mappedTrackInfo != null && willHaveContent(
                mappedTrackInfo
            )
        }
